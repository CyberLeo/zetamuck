...
/* dbref is used as the target of any PROG_LOCK objects encountered */
...
/* helper functions for other programs to call */

char *
array_jencode(stk_array *input, int flags, dbref player) {
    json_t *newjson;
    char *jsontext;
    
    if (!(newjson = array_to_json(input, player))) {
        return NULL;
    }
    
    if (!(jsontext = json_dumps(newjson, flags))) {
        return NULL;
    }
...
stk_array *
jdecode_array(char *input, int flags, stk_array **errarr) {
    json_t *newjson;
    json_error_t error;
    stk_array *jsonarr;

    newjson = json_loads(input, flags, &error);
...
stk_array *
array_jdecode(stk_array *input, int flags, stk_array **errarr) {
    struct inst temp1;
    struct inst *in;
    char buf[BUFFER_LEN * 2];

    if (!array_is_homogenous(input, PROG_STRING)) {
        *errarr = NULL;
        return NULL;
    }

    if (array_first(input, &temp1)) {
       buf[0] = '\0';
       do {
           /* REDO: rework this to be less lazy later */
           in = array_getitem(input, &temp1);
           sprintf(buf, "%s%s\n", buf, DoNullInd(in->data.string));
       } while (array_next(input,&temp1));
    } else {
        *errarr = NULL;
        return NULL;
    }

    return jdecode_array(buf, flags, errarr);
}

#ifdef FILE_PRIMS

stk_array *
jdecode_file(char *filename, int flags, stk_array **errarr) {
    json_t *newjson;
    json_error_t error;
    stk_array *jsonarr;
...
int
jencode_file(stk_array *input, char *filename, int flags, dbref player) {
    json_t *jsonout;
    int success;

    jsonout = array_to_json(input, player);
